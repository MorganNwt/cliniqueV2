    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']) {
            $_POST = filter_input_array(INPUT_POST, [
                ...
                'cg_accepted' => FILTER_VALIDATE_BOOLEAN,
            ]);
            ...
            $email = $_POST['email'];
            $passwd = $_POST['passwd'];
            $cg_accepted = $_POST['cg_accepted'];

            // Check if the cg_accepted checkbox is checked
            if (!$cg_accepted) {
                // If cg_accepted is not accepted, stop the registration process
                header('Location: inscription.php?error=cg_required');
                exit();
            }
            ...
            try {
                ...
                // Insert user into users table, including the cg_accepted field
                $requete_users = 'INSERT INTO users (pseudo, email, passwd, cg_accepted) VALUES (:pseudo, :email, :passwd, :cg_accepted)';
                ...
                $stmt_users->bindParam(':cg_accepted', $cg_accepted, PDO::PARAM_BOOL);
                $stmt_users->execute();
